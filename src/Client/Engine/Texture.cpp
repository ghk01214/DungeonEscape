...
void CTexture::Init(const std::wstring& path)
{
	CreateTexture(path);
	CreateView();
}

void CTexture::CreateTexture(const std::wstring& path)
{
	// 파일 확장자 얻기
	std::wstring ext = fs::path(path).extension();

	// 확장자에 따라 다른 포맷으로 이미지 로드
